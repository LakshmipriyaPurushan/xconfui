...
rm -rf $HOME/scan_repo  $GITHUB_WORKSPACE/xconf_snyk_vulnerability_reports
...
mkdir -p "$GITHUB_WORKSPACE/xconf_snyk_vulnerability_reports"

REPORT_FILE="$GITHUB_WORKSPACE/xconf_snyk_vulnerability_reports/xconfui_snyk_audit_results.txt"

echo "Running go, writing report to: $REPORT_FILE"

# Run snyk and capture all output
set +e  # don't exit immediately if govulncheck finds issues
snyk code test > "$REPORT_FILE" 2>&1
status=$?
set -e

echo "npm audit exited with status: $status"
ls -l "$GITHUB_WORKSPACE/xconf_snyk_vulnerability_reports"
